...
public class DocumentUI {
	...
	private void activateDocumentViewer() {
		try {
			...
			JOptionPane.showMessageDialog(frame, "Cannot read document", 					"IO Error", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void createActionsPanel(JPanel documentPanel) {			JPanel actionsPanel = new JPanel();
		...
		actionsPanel.add(pageActionLabel);				JLabel closeLabel = new JLabel(new ImageIcon("images/close.png"));
	...
	public void updateBookmarkLabel(int pageN, boolean active) {
		...
		return new MouseAdapter () {
			...
			public void mouseClicked(MouseEvent event) {
				if (event.getButton() == MouseEvent.BUTTON1)         			pageDown();
        		else if(event.getButton() == MouseEvent.BUTTON3)         			pageUp();
			}
			};
	}
